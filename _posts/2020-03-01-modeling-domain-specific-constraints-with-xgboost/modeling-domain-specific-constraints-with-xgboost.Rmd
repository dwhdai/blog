...
inr <- 1 + 0.3*gender + 0*lvef_50 + 0.25*warfarin + 0.2*lvef_50*warfarin + 1.5*gender*lvef_50*warfarin + rnorm(mean = 0.25, n)
...
shap_vals_original <- predict(xgb_fit_original, 
                              ...
                              predcontrib = T) %>% 
  as_tibble() %>% 
  rename(gender_shap = gender,
         warfarin_shap = warfarin,
         bias_shap = BIAS) %>% 
  bind_cols(test) %>% 
...
shap_vals <- predict(xgb_fit, 
  ...
  bind_cols(test) %>% 
  bind_cols(pred = pred) %>% 
  mutate(experiment = "Unconstrained Model")
...
```{r monotonic vs unconstrained model}
