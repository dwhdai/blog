---
title: "Modeling domain-specific constraints with xgboost"
description: |
  Incorporating domain knowledge from subject matter experts is critical for building useful models. As statistician George Box said, *"All models are wrong, but some are useful."* How useful our models are depend critically on how well we apply domain knowledge to extract the signal from the noise.
author:
  - name: David Dai
    url: https://www.daviddai.ca/
    affiliation: Data Science & Advanced Analytics, Unity Health Toronto
    affiliation_url: https://www.chartdatascience.ca/david-dai
date: "`r Sys.Date()`"
output:
  distill::distill_article:
    self_contained: false
draft: true
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE,
                      warning = F,
                      message = F,
                      fig.keep = "all")
```

# Current State

Recent methodological advances in conjunction with the wealth of available data has raised questions about the importance of subject matter experts and their domain expertise when it comes to building prediction models. The popularity of feature engineering, once considered of extreme critical importance in building the most performant prediction models, has taken a back-seat to give way to models like deep learning that put less emphasis on explicit user-defined features.

While this may be a viable strategy in certain use-cases (eg. big-tech companies that have a lot of data, intentionally collected for specific purposes, and can validate results prospectively using experimentation), the current state and foreseeable future of prediction modeling in healthcare and clinical medicine requires a more collaborative approach between the domain experts (doctors, nurses, clinical managers, hospital executives) and data analysts.

# 




# Misc

<blockquote>Given the rough
medical truism that serious diseases are less common than mild diseases, it is even likely that
underperformance in minority subsets could lead to disproportionate harm to patients. - Oakden-Rayner et al., NeurIPS 2019</blockquote>

With increased capabilities of modern day algorithms to extract signal from noisy data, as well as increased awareness of said algorithms, it is paramount that we not forget the purpose and art of model-building: to create a mathematical representation of *real life*. 




# A Simulated Experiment

```{r}
library(tidyverse)
library(xgboost)
library(ggplot2)
```


```{r cache = T}
# Simulate some data

set.seed(112892)

n <- 50000

gender <- rbinom(n = n, size = 1, prob = 0.5)
lvef_50 <- rbinom(n = n, size = 1, prob = 0.5)
warfarin <- rpois(lambda = 28, n = n)/4

# inr <- 1 + 0.3*gender + 0*lvef_50 + 0.25*warfarin + 0.5*lvef_50*warfarin +
#   0.5*gender*warfarin  + rnorm(mean = 0.25, n)

inr <- 1 + 0.3*gender + 0*lvef_50 + 0.25*warfarin + 0.2*lvef_50*warfarin + 0.7*gender*lvef_50*warfarin + rnorm(mean = 0.25, n)


df <- tibble(
  gender = gender,
  lvef_50 = lvef_50,
  warfarin = warfarin,
  inr = inr
) %>% 
  # Create sampling weight for artificial sample bias
  mutate(sampling_weight = case_when(
    gender == 1 & lvef_50 == 1 & warfarin > 9 ~ 0,
    gender == 0 & lvef_50 == 1 & warfarin > 9~ 0.9,
    gender == 0 & lvef_50 == 1 & warfarin <= 9 ~ 0.01,
    lvef_50 == 0 & warfarin <= 9 ~ 0.01,
    gender == 0 ~ 0.02,
    TRUE ~ 1
  ))

# Create a biased dataset with the sample size
set.seed(10000)
bias_df <- df %>% 
  sample_frac(1,
              replace = TRUE,
              weight = sampling_weight) 

```

```{r plot of original data}
theme_set(theme_bw() + 
            theme(strip.text = element_text(size = 14),
                  axis.title = element_text(size = 14),
                  panel.grid.minor = element_blank()))

df %>% 
  mutate(
    gender = case_when(
      gender == 0 ~ "Male",
      gender == 1 ~ "Female"
    ),
    lvef_50 = case_when(
      lvef_50 == 0 ~ "LVEF >50%",
      lvef_50 == 1 ~ "LVEF <=50%"
    )) %>% 
  ggplot(aes(x = warfarin,
             y = inr)) +
  stat_smooth() +
  geom_point() +
  labs(y = "Blood Thinness Index (INR)",
       x = "Blood Thinner Dosage (Warfarin, mg)")

df %>% 
  mutate(
    gender = case_when(
      gender == 0 ~ "Male",
      gender == 1 ~ "Female"
    ),
    lvef_50 = case_when(
      lvef_50 == 0 ~ "LVEF >50%",
      lvef_50 == 1 ~ "LVEF <=50%"
    )) %>% 
  ggplot(aes(x = warfarin,
             y = inr)) +
  stat_smooth() +
  geom_point() +
  facet_grid(gender ~ lvef_50) +
  labs(y = "Blood Thinness Index (INR)",
       x = "Blood Thinner Dosage (Warfarin, mg)")

```

```{r}
bias_df %>% 
  mutate(
    gender = case_when(
      gender == 0 ~ "Male",
      gender == 1 ~ "Female"
    ),
    lvef_50 = case_when(
      lvef_50 == 0 ~ "LVEF >50%",
      lvef_50 == 1 ~ "LVEF <=50%"
    )) %>% 
  ggplot(aes(x = warfarin,
             y = inr)) +
  stat_smooth() +
  geom_point(alpha = 0.05) +
  labs(y = "Blood Thinness Index (INR)",
       x = "Blood Thinner Dosage (Warfarin, mg)")

bias_df %>% 
  mutate(
    gender = case_when(
      gender == 0 ~ "Male",
      gender == 1 ~ "Female"
    ),
    lvef_50 = case_when(
      lvef_50 == 0 ~ "LVEF >50%",
      lvef_50 == 1 ~ "LVEF <=50%"
    )) %>% 
  ggplot(aes(x = warfarin,
             y = inr)) +
  stat_smooth() +
  geom_point(alpha = 0.05) +
  facet_grid(gender ~ lvef_50) +
  labs(y = "Blood Thinness Index (INR)",
       x = "Blood Thinner Dosage (Warfarin, mg)")
```


```{r fit model, cache = T}
# Calculate row index for data splits
train_ind <- 0.7*n
valid_ind <- 0.85*n

# Original data
train <- df %>%
  dplyr::slice(1:train_ind)

valid <- df %>%
  dplyr::slice((train_ind + 1):valid_ind)

test <- df %>% 
  dplyr::slice((valid_ind + 1):n)

# Biased data
train_bias <- bias_df %>% 
  dplyr::slice(1:train_ind)

valid_bias <- bias_df %>%
  dplyr::slice((train_ind + 1):valid_ind)

test_bias <- bias_df %>% 
  dplyr::slice((valid_ind + 1):n)

# Create xgboost matrices
train_mat <- xgb.DMatrix(train %>% 
                           select(-inr,
                                  -lvef_50,
                                  -sampling_weight) %>% 
                           as.matrix(), 
                         label = train$inr)

train_mat_bias <- xgb.DMatrix(train_bias %>% 
                                select(-inr,
                                       -lvef_50,
                                       -sampling_weight) %>% 
                                as.matrix(), 
                              label = train_bias$inr)

valid_mat <- xgb.DMatrix(valid %>% 
                           select(-inr,
                                  -lvef_50,
                                  -sampling_weight) %>% 
                           as.matrix(), 
                         label = valid$inr)

valid_mat_bias <- xgb.DMatrix(valid_bias %>% 
                                select(-inr,
                                       -lvef_50,
                                       -sampling_weight) %>% 
                                as.matrix(), 
                              label = valid_bias$inr)

test_mat <- xgb.DMatrix(test %>% 
                          select(-inr,
                                 -lvef_50,
                                 -sampling_weight) %>% 
                          as.matrix(), 
                        label = test$inr)

test_mat_bias <- xgb.DMatrix(test_bias %>% 
                               select(-inr,
                                      -lvef_50,
                                      -sampling_weight) %>% 
                               as.matrix(), 
                             label = test_bias$inr)

```


```{r model with original dataset, cache = T}
# Train model with original dataset
watchlist <- list(train = train_mat,
                  eval = valid_mat)

params <- list(max_depth = 3,
               eta = 0.03, 
               nthread = 4,
               eval_metric = "rmse",
               objective = "reg:squarederror")

xgb_fit_original <- xgb.train(data = train_mat, 
                              params = params,
                              nrounds = 1000,
                              verbose = F,
                              early_stopping_rounds = 50,
                              watchlist = watchlist)

# Generate predictions
pred_original <- predict(xgb_fit_original, test_mat)

shap_vals_original <- predict(xgb_fit_original, 
                              test_mat,
                              predcontrib = T) %>% 
  as_tibble() %>% 
  rename(warfarin_shap = warfarin,
         bias_shap = BIAS) %>% 
  bind_cols(test) %>% 
  bind_cols(pred = pred_original) %>% 
  mutate(experiment = "Original")


```



```{r model with biased dataset, cache = T}
# Train model with biased dataset
watchlist_bias <- list(train = train_mat_bias,
                       eval = valid_mat_bias)

params <- list(max_depth = 3,
               eta = 0.03, 
               nthread = 4,
               eval_metric = "rmse",
               objective = "reg:squarederror")

xgb_fit <- xgb.train(data = train_mat_bias, 
                     params = params,
                     nrounds = 1000,
                     verbose = F,
                     early_stopping_rounds = 50,
                     watchlist = watchlist)

xgb_fit_monotonic <- xgb.train(data = train_mat_bias,
                               params = params,
                               monotone_constraints = c(0,1),
                               nrounds = 1000,
                               verbose = F,
                               early_stopping_rounds = 50,
                               watchlist = watchlist)

# Generate predictions
pred <- predict(xgb_fit, test_mat)
pred_monotonic <- predict(xgb_fit_monotonic, test_mat)

shap_vals <- predict(xgb_fit, 
                     test_mat,
                     predcontrib = T) %>% 
  as_tibble() %>% 
  rename(warfarin_shap = warfarin,
         bias_shap = BIAS) %>% 
  bind_cols(test) %>% 
  bind_cols(pred = pred) %>% 
  mutate(experiment = "Unconstrained")

shap_vals_monotonic <- predict(xgb_fit_monotonic, 
                               test_mat,
                               predcontrib = T) %>% 
  as_tibble() %>% 
  rename(warfarin_shap = warfarin,
         bias_shap = BIAS) %>% 
  bind_cols(test) %>% 
  bind_cols(pred = pred_monotonic) %>% 
  mutate(experiment = "Monotonic")

plot_data <- bind_rows(shap_vals_original,
                       shap_vals,
                       shap_vals_monotonic)

```


```{r monotonic vs unconstrained model, fig.height = 5, fig.width = 7}

ggplot(data = plot_data %>% 
         mutate(experiment = factor(experiment,
                                    levels = c("Original",
                                               "Unconstrained",
                                               "Monotonic"))), 
       aes(x = warfarin,
           y = pred)) +
  # geom_point()
  stat_smooth() +
  facet_grid(experiment ~ .) +
  coord_cartesian(xlim = c(0, 15)) + 
  labs(x = "Blood Thinner Dosage (Warfarin, mg)",
       y = "Blood Thinness Index (INR)")


```

# Conclusion

In reality, data analysis aims to elucidate the data generating mechanism, and in a world where the data are vast and noise is abundant, the understanding of subject domain and its quirks is crucial for creating a data analysis with utility. 